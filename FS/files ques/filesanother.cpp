...
int main()
{
	...
	addr ad;
	char ch;
	cout<<" how many employees' details do you want to save ? "<<endl;
	cin>>n;
	cout<<"enter employee details as employee no,name"<<endl;
	cin.ignore();
	for(int i=0;i<n;i++)
	...
	{ 
	  ...
	  cout<<"enter employee no:"<<endl;
		...
		pack(buffer,s.name);
		cout<<"do you want to give your mobile number or room no? enter 1 for m.no and 2 for house no ::"<<endl;
		cin>>s.tag;
		cin.ignore();
		if(s.tag==1)
		{   
		...
		{ 
		cout<<"enter your house no .(ex 14-56) :(start with 0)"<<endl;
	cin.ignore();
