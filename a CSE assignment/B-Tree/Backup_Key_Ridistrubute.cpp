...
int Key_Redis(bdnode *B,int added_into,int added,bdnode* &New)
{
	int i;
	...
	if(added_into==2*d)
	{
		if(B->ptr[added_into-1]->c<2*d)
		{
			...
			int min=T->data[0];
			for(i=0;i<(d-1);i++)
			T->data[T->c++]=T->data[i+1];
			Insert_and_Sort(B->ptr[added_into-1],B->data[added_into-1]);
			B->data[added_into-1]=min;
			T->data[T->c++]=added;
			for(i=d;i<2*d;i++)
			T->data[T->c++]=New->data[i-d];
			return 1;
		}
		else
		return 0;
	}
	if(B->c>added_into&&B->ptr[added_into+1]->c<2*d)
	{
		T->c=0;
		int max=New->data[d-1];
	...
	else
	{
		...
		else                    //search for leafnode
		{
			...
			if(added!=-1)
			{
				...
				{
					success=Key_Redis(B,added_into,added,New);
				}
