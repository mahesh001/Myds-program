...
void Overflow(bdnode *B,int added_into)
{
	...
	int success=0,non_full_node;
	int i;
	cout<<"B->c="<<B->c<<endl;
	if(added_into==0)
	{
		for(i=1;i<=B->c;i++)    //checking for nonfull sibling
		{
			if(B->ptr[i]->c<2*d)
			{
				success=1;
		...
		{
			non_full_node=i-1;
			for(i=non_full_node;i>=0;i--)   //distrubution
			{
	...
	{
		for(i=added_into-1;i>=0;i--)     //checking for key_redis in lower value
		{
			if(B->ptr[i]->c<2*d)
			{
				success=1;
		...
		if(i>=0)
		{
			cout<<"IN_lowervalue_key_redis\n";
			non_full_node=i;
			for(i=non_full_node;i<=added_into-1;i++)
			{
				Right_to_left_rotation(B,i);
			}
		}
		if(success==0)   //if key in above not possi. then check for higher element
		{
			for(i=added_into+1;i<=B->c;i++)
			{
				if(B->ptr[i]->c<2*d)
				{
					success=1;
			...
			if(i<=B->c)
			{
				cout<<"IN_highvalue_key_redis\n";
				non_full_node=i-1;
				for(i=non_full_node;i>=added_into;i--)
				Left_to_right_rotation(B,i);
			}
		}
	}
...
void Add(bdnode* &B,int k)
{
	...
	{	
		...
		{
			cout<<"added_into="<<added_into<<endl;
