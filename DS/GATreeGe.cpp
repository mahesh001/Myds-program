// $Header: /usr/people/mbwall/src/galib/ga/RCS/GATreeGenome.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  tree.C
  mbwall 25feb95
...
GATreeGenome<T>::copy(const GAGenome & orig)
{
  ...
  GATreeGenome<T> & c = (GATreeGenome<T> &)orig;
  GAGenome::copy(orig);
  GATree<T>::copy(c);
}
...
_tt(ostream & os, GANode<T> * n)
{
  ...
  for(GANodeBASE * tmp=node->next; tmp && tmp != node; tmp=tmp->next){
    ...
    os.width(10); os << &(((GANode<T> *)tmp)->contents) << "\n";
    _tt(os, (GANode<T> *)tmp->child);
  }
}
...
{
  if(this == &c) return 1;
  GATreeGenome<T>& b = (GATreeGenome<T> &)c;
...
{
  GATreeGenome<T> &child=(GATreeGenome<T> &)c;
  register int n, i;
  if(pmut <= 0.0) return 0;

  n = child.size();
  float nMut = pmut * (float)n;
...
{
  if(&a == &b) return 0;
  GATreeGenome<T>& sis=(GATreeGenome<T>&)a;
  GATreeGenome<T>& bro=(GATreeGenome<T>&)b;

  return _GATreeCompare(sis.rt, bro.rt);
}
...
OnePointCrossover(const GAGenome& p1, const GAGenome& p2, 
  ...
  GATreeGenome<T> &mom=(GATreeGenome<T> &)p1;
  GATreeGenome<T> &dad=(GATreeGenome<T> &)p2;
  ...
  if(c1 && c2){
    GATreeGenome<T> &sis=(GATreeGenome<T> &)*c1;
    GATreeGenome<T> &bro=(GATreeGenome<T> &)*c2;
