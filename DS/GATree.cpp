// $Header: /usr/people/mbwall/src/galib/ga/RCS/GATree.C,v 1.2 1996/11/13 16:53:57 mbwall Exp mbwall $
/* ----------------------------------------------------------------------------
  treetmpl.C
  mbwall 25feb95
...
{
  _GATreeDestroy((GANode<T> *)rt);
  rt = _GATreeCopy((GANode<T> *)orig.rt, (GANode<T> *)0);
  iter.node = rt;
...
GATree<T>::remove()
{
  ...
  GANode<T> * node = (GANode<T> *)iter.node;
  if(!node) return t;
...
GATree<T>::clone(unsigned int i) const
{
  ...
  GANode<T> * node;
  unsigned int w=0;
  if(i == 0) node = (GANode<T> *)rt;
  else node = (GANode<T> *)_GATreeTraverse(i, w, rt);
  if(!node) return t;

  GANode<T> * newnode = new GANode<T>(node->contents);
  newnode->child = _GATreeCopy((GANode<T> *)node->child, newnode);
...
GATree<T>::destroy()
{
  ...
  if(node->prev == node || !node->prev)
    ...
    iter.eldest();
  _GATreeDestroy((GANode<T> *)node->child);
...
//      tmp->prev = tmp;
      ...
      if(insert((GANode<T> *)tmp, (GANode<T> *)0, GATreeBASE::ROOT) ==
	 GATreeBASE::ERR)
	return GATreeBASE::ERR;
    }
  }
...
//      tmp->prev = tmp;
      iter.node = (GANodeBASE *)0;
      if(t->insert((GANode<T> *)tmp,(GANode<T> *)0,GATreeBASE::ROOT) ==
	 GATreeBASE::ERR)
	return GATreeBASE::ERR;
    }
  }
...
_GATreeCopy(GANode<T> * node, GANode<T> * parent)
{
  ...
  GANode<T> *tmp = (GANode<T> *)node->next;
  while(tmp && tmp != node){
    newtmp = new GANode<T>(tmp->contents);
    newtmp->parent = parent;
    newtmp->child = _GATreeCopy((GANode<T> *)tmp->child, newtmp);
    newtmp->prev = lasttmp;
    lasttmp->next = newtmp;

    lasttmp = newtmp;
    tmp = (GANode<T> *)tmp->next;
  }
...
_GATreeDestroy(GANode<T> * node)
{
  ...
  while(node->next && node->next != node){
    ...
    node->next = tmp->next;
    tmp->next->prev = node;
    _GATreeDestroy((GANode<T> *)tmp->child);
    delete tmp;
  }
  delete node;
}
